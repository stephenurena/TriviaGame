...
//==========================================================================================
var game = {
	//start page with button to start game
	...
	//general question ans answer markup
	gMarkUp: function(){
	...
	//timer to count down time by a second and add to html
	...
	//on a loss win or unanswered this function helps navigate to reset new question
	inBetweenScreen: function () {
		if (questionCounter < 6) {
			$("#quiz").show();
			//tried to hide for #quizWin but didn't work, was hiding during a win
			questionCounter++;
			clearInterval(clockCount);
			this.gMarkUp();
			counter = 10;
	...
	//when a user doesn't anwswer in the allotted time
	unansweredLoss: function() {
		lossTally++;
		// console.log(lossTally);
		setInterval(this.inBetweenScreen(), 1000 *10);
	},
	//on a win shows an updated paragraph and video to correct answer.
	win: function(){
		winTally++;
	...
	//will reset game without reloading the page
	reset: function () {
		$("#results").hide();
		questionCounter = 0;
		winTally = 0;
...
//on click to answer a selection with the class of answer
